  ...
  return (
    <section className="rounded-3xl border border-slate-800 bg-slate-900/90 p-6 shadow-xl shadow-slate-950/25">
      <div className="mb-5 flex items-center justify-between gap-4">
        <div>
          <h2 className="text-xl font-semibold text-emerald-300">Leaderboard</h2>
          <p className="text-sm text-slate-400">See how users rank by sustainability score.</p>
        </div>
        <span className="rounded-full bg-emerald-500/15 px-3 py-1 text-xs text-emerald-200">Top users</span>
      </div>
      <div className="space-y-3">
        {leaders.length > 0 ? (
          ...
          <p className="rounded-3xl border border-slate-800 bg-slate-950/80 px-5 py-4 text-sm text-slate-400">No leaderboard data yet.</p>
        )}
      </div>
    </section>
  );
